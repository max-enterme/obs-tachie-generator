...
2. **② 立ち絵・演出プリセット**：再利用する見た目を作る。**画像**（アップロード→data URI 埋め込み or 画像URL）＋位置(left/bottom)・幅(width)＋発話演出（ぴょこぴょこ/枠・後光〈色指定可〉/点滅）＋静かな人を暗くする。複数作れます。
3. **③ 組み合わせ & プレビュー**：**ユーザー × プリセット**のペアを作る。ペア行でユーザーを差し替え可（テスト用ID↔本番ID）。プレビューで確認。
4. **④ 出力**：**ペアごと**の CSS を **コピー** or **ダウンロード**。
