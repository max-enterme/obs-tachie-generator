...
- [ ] T12: GUI モックの併置（SPEC-OPS §10）は**実施せず**。spec 段階でモックを置く代わりに、実装しながら
      ブラウザ実測（位置・幅・文字サイズ・帯）で挙動を確定し、確定値を spec.md / plan.md に反映した。
      次回の GUI feature では §10 どおり spec 作成時にモックを併置する ※方針の記録（作業タスクではない）
- [ ] T8: OBS 実機で貼って確認(名前の見え方・フォント可用性・`:has()` 挙動) ※実機確認(§08)
