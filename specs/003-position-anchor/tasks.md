...
- [ ] T1: 型と既定値 — `GenerateOptions` / `Preset` に `anchorX`/`anchorY`(任意)を追加、`presetToOptions` で受け渡し、保存済みデータの欠損補完  <!-- #12 -->
- [ ] T2: `generateCss` の位置生成をアンカー対応に — 位置宣言の切り出し + `left`/`right`/`top`/`bottom`/`50%` の出し分け + vitest(9通り・左下は従来出力と一致)  <!-- #13 -->
- [ ] T3: `transform` 衝突の解消 — 中央寄せ translate を keyframes に前置する形へ。中央寄せ × ぴょこぴょこの vitest  <!-- #14 -->
